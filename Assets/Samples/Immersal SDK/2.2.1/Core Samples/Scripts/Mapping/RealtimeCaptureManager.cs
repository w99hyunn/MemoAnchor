...
namespace Immersal.Samples.Mapping
{
    ...
    {
        // --- 추가된 UI 변수들 ---
        [Header("Custom UI Settings")]
        [SerializeField] private GameObject m_ScanningPanel; // "스캔 중입니다" 이미지/패널
        [SerializeField] private GameObject m_SaveCompletePanel; // "저장되었습니다" 이미지/패널
        [SerializeField] private Button m_SaveButton; // 저장 버튼 (StopRealtimeCapture 호출용)
        ...
        private void Start()
        {
            ...
            if (m_SaveButton) m_SaveButton.gameObject.SetActive(false);
        ...
        public async void StartRealtimeCapture()
        {
            if (Application.platform == RuntimePlatform.IPhonePlayer || Application.platform == RuntimePlatform.Android)
            {
                ...
                if (m_SaveButton) m_SaveButton.gameObject.SetActive(true);
                if (m_SaveCompletePanel) m_SaveCompletePanel.SetActive(false);
        ...
        public async void StopRealtimeCapture()
        {
            ...
            {
                Debug.LogError("홈 씬 이름이 설정되지 않았습니다!");
            }


        }

        private void HideSaveCompletePanel()
        {
            if (m_SaveCompletePanel) m_SaveCompletePanel.SetActive(false);
        }
