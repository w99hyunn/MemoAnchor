...
public class ARKitMeshScanController : MonoBehaviour
{
    ...
    [SerializeField] private bool enhancePreviewWithGemini = true;
    ...
    private void Awake()
    {
        ...
        if (!arOcclusionManager)
            arOcclusionManager = arCamera ? arCamera.GetComponent<AROcclusionManager>() : FindFirstObjectByType<AROcclusionManager>();
    ...
    public void StopScanAndShowMap()
    {
        ...
        if (packageReconstructionScanOnStop && !reconstructionPackageRunning)
            StartCoroutine(PackageAndMaybeUploadReconstructionScan(bounds.Value));

        if (enhancePreviewWithGemini && !geminiEnhancementRunning)
            StartCoroutine(EnhancePreviewWithGemini(bounds.Value));
    ...
    private IEnumerator PackageAndMaybeUploadReconstructionScan(Bounds bounds)
    {
        ...
        if (uploadReconstructionPackageOnStop && !string.IsNullOrWhiteSpace(reconstructionUploadUrl))
            yield return UploadReconstructionPackage(zipPath);
    ...
    private IEnumerator UploadReconstructionPackage(string zipPath)
    {
        ...
        var uploadUrl = reconstructionUploadUrl.Trim();
        if (!Uri.TryCreate(uploadUrl, UriKind.Absolute, out _))
        {
            SetExportStatus("Upload URL is invalid. Package saved locally.");
            Debug.LogWarning($"[ARKitMeshScanController] Invalid reconstruction upload URL: '{reconstructionUploadUrl}'");
            yield break;
        }
        ...
        using (var request = new UnityWebRequest(uploadUrl, UnityWebRequest.kHttpVerbPOST))
        {
            ...
            if (request.result == UnityWebRequest.Result.Success)
            {
                ...
                yield return PollReconstructionStatus(serverScanId);
            }
            else
            {
    ...
    private IEnumerator PollReconstructionStatus(string serverScanId)
    {
        if (string.IsNullOrWhiteSpace(serverScanId))
            yield break;

        var statusUrl = BuildReconstructionEndpointUrl("status", serverScanId);
        ...
        for (var attempt = 0; attempt < 90; attempt++)
        {
            using (var request = UnityWebRequest.Get(statusUrl))
            {
                ...
                {
                    var resultUrl = BuildReconstructionEndpointUrl("result", serverScanId);
                    ...
                    yield return DownloadAndShowReconstructionResult(resultUrl, status.resultFile, serverScanId);
                    yield break;
                }
    ...
    private IEnumerator DownloadAndShowReconstructionResult(string resultUrl, string resultFile, string serverScanId)
    {
    ...
    private IEnumerator EnhancePreviewWithGemini(Bounds bounds)
    {
        ...
        yield return client.GenerateSemanticMap(
            scanPackage,
            images,
            (json, result) =>
            {
    ...
    private GeminiSemanticMapClient GetOrCreateGeminiClient()
    {
        ...
        geminiClient = GetComponent<GeminiSemanticMapClient>();
        if (!geminiClient)
