...
iPad Unity app
  ...
  -> POST authenticated map reconstruction API to ASP.NET Core

ASP.NET Core
  -> validates Unity JWT and map manager/member access
  -> streams upload/status/result requests to 127.0.0.1:8765
...
On the Linux host, install the included unit after copying the repository tools
to `/home/ubuntu/MemoAnchorReconstruction`:

```bash
sudo cp tools/reconstruction_server/memoanchor-reconstruction.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now memoanchor-reconstruction.service
```

The production app does not set `reconstructionUploadUrl`. `MapScanSession`
builds the authenticated ASP.NET Core path from the created map ID, and
`ServicesManager` supplies the public server origin.
...
The ASP.NET Core proxy polls internally:
...
python3.10 server.py --host 127.0.0.1 --port 8765
```

Set the Unity `ARKitMeshScanController` fields:

- `uploadReconstructionPackageOnStop`: enabled
- `reconstructionUploadUrl`: blank for the integrated Main flow
